...
jQuery.fn.stringToSlug = function(options) { var defaults = { setEvents: 'keyup keydown blur', getPut: '#permalink', space: '-', prefix: '', suffix: '', replace: '', AND: 'and', callback: false, }; var opts = jQuery.extend(defaults, options);
    jQuery(this).bind(defaults.setEvents, function() { var text = jQuery(this).val();
        text = defaults.prefix + text + defaults.suffix;
        text = text.replace(defaults.replace, "");
        text = jQuery.trim(text.toString()); var chars = []; for (var i = 0; i < 32; i++) { chars.push('') }
        ...
        jQuery(defaults.getPut).html(stringToSlug); if (defaults.callback != false) { defaults.callback(stringToSlug) } return this }); return this };